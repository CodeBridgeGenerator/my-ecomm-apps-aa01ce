  ...
  const [isHelpSidebarVisible, setHelpSidebarVisible] = useState(false);
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((res) => {
                set_entity(res || {});
                const createdBy = Array.isArray(res.createdBy)
            ? res.createdBy.map((elem) => ({ _id: elem._id, email: elem.email }))
            : res.createdBy
                ? [{ _id: res.createdBy._id, email: res.createdBy.email }]
                : [];
...
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Limit</label><p className="m-0 ml-3" >{Number(_entity?.limit)}</p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">RedeemedCount</label><p className="m-0 ml-3" >{Number(_entity?.redeemedCount)}</p></div>
            <div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">CreatedBy</label>
                    {createdBy.map((elem) => (
